...
def galex(band='fuv', ra_ctr=None, dec_ctr=None, size_deg=None, index=None, name=None, write_info=True):
    ...
    if name == 'NGC0024':
        ...
        for indir in [input_dir, converted_dir, masked_dir, reprojected_dir, weights_dir, weighted_dir, final_dir]:
            os.makedirs(indir)
        ...
        int_infiles = glob.glob(os.path.join(input_dir, '*-int.fits'))
        wt_infiles = glob.glob(os.path.join(input_dir, '*-rrhr.fits'))

        int_outfiles = [os.path.join(converted_dir, os.path.basename(f).replace('.fits', '_mjysr.fits')) for f in int_infiles]
        wt_outfiles = [os.path.join(converted_dir, os.path.basename(f).replace('.fits', '.fits')) for f in wt_infiles]
    ...
    #try:
        ...
        int_suff, rrhr_suff = '*_mjysr_masked.fits', '*-rrhr_masked.fits'
        int_images = sorted(glob.glob(os.path.join(masked_dir, int_suff)))
        rrhr_images = sorted(glob.glob(os.path.join(masked_dir, rrhr_suff)))
        reproject_images(hdr_file, int_images, rrhr_images, masked_dir, reprojected_dir)


        # COADD IMAGES TO SEE HOW MANY OVERLAPS EACH PIXEL HAS
        temp_dir = os.path.join(gal_dir, 'temp_coadd')
        os.makedirs(temp_dir)
        infiles = glob.glob(os.path.join(reprojected_dir, '*_mjysr_masked*.fits'))
        for infile in infiles:
            basename = os.path.basename(infile)
            new_in_file = os.path.join(temp_dir, basename)
            os.symlink(infile, new_in_file)
        table = create_tables(temp_dir, table_type='temp')
        coadd(hdr_file, temp_dir, temp_dir, output='temp', add_type='count')
        temp_coaddfile = os.path.join(temp_dir, 'temp_mosaic.fits')
        ...
        weight_images(imfiles, wtfiles, temp_coaddfile, weighted_dir, weights_dir)

        # CREATE THE METADATA TABLES NEEDED FOR COADDITION
        table1 = create_tables(weights_dir, table_type='weights')
        table2 = create_tables(weighted_dir, table_type='int')
...
def reproject_images(template_header, int_images, rrhr_images, input_dir, reprojected_dir, whole=False, exact=True):
...
def coadd(template_header, output_dir, input_dir, output=None, exact=True, add_type='mean'):
    img_dir = input_dir
    if output is None:
        out_table = 'reprojected.tbl'
        out_image = os.path.join(output_dir, 'mosaic.fits')
    else:
        out_table = output + '_reprojected.tbl'
        out_image = os.path.join(output_dir, output + '_mosaic.fits')

    reprojected_table = os.path.join(img_dir, out_table)
    out_image = os.path.join(output_dir, output + '_mosaic.fits')

    montage.mAdd(reprojected_table, template_header, out_image, img_dir=img_dir, exact=exact, type=add_type)


def weight_images(imfiles, wtfiles, tempfile, weighted_dir, weights_dir):
    for i in range(len(imfiles)):
        imfile = imfiles[i]
        wtfile = wtfiles[i]
        im, hdr = pyfits.getdata(imfile, header=True)
        rrhr, rrhrhdr = pyfits.getdata(wtfile, header=True)
        tmp = pyfits.getdata(tempfile)
